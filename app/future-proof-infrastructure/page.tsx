...
const href = "/future-proof-infrastructure";

export async function generateMetadata(): Promise<Metadata> {
  const service = getServiceByHref(href);
  return {
    title: service ? `${service.title} | ITEK Solutions` : "Future-proof Your Infrastructure | ITEK Solutions",
    description: service?.shortDescription ?? service?.description ?? undefined,
  };
}

export default function FutureProofInfrastructurePage() {
  const service = getServiceByHref(href);
