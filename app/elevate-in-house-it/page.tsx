...
const href = "/elevate-in-house-it";

export async function generateMetadata(): Promise<Metadata> {
  const service = getServiceByHref(href);
  return {
    title: service ? `${service.title} | ITEK Solutions` : "Elevate Your In-House IT | ITEK Solutions",
    description: service?.shortDescription ?? service?.description ?? undefined,
  };
}

export default function ElevateInHouseItPage() {
  const service = getServiceByHref(href);
